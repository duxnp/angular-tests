...
export function doesServiceImplementsOnDestroy<S>(
  testedService: S
): testedService is S & OnDestroy {
  return (
    'ngOnDestroy' in testedService &&
    typeof (testedService as S & OnDestroy)['ngOnDestroy'] === 'function'
  );
}
...
export function isHTMLOptionElementArray(
  value: any
): value is HTMLOptionElement[] {
  return (
    Array.isArray(value) &&
    !!value.length &&
    value.every((item) => item instanceof HTMLOptionElement)
  );
}
