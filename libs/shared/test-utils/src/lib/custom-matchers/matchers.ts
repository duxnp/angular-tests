...
/* eslint-disable @typescript-eslint/no-explicit-any, no-prototype-builtins,@typescript-eslint/ban-ts-comment  */
// @ts-nocheck
...
const checkProperty = (
  ...
  for (const key of Object.keys(prop)) {
    ...
    const addendum =
      prop[key] !== undefined ? ` with value '${prop[key]}'` : '';
  ...
  const message = () =>
    `Expected element${pass ? ' not' : ''} to have property ${failing}`;
...
const hasSameText = (
  ...
  if (expected && Array.isArray(expected)) {
    ...
    const message = () =>
      `Expected element${pass ? ' not' : ''} to have ${
        exact ? 'exact' : ''
      } text '${failing}', but had '${actual}'`;
  ...
  if (expected && typeof expected !== 'string') {
    ...
    const message = () =>
      `Expected element${pass ? ' not' : ''} to have ${
        exact ? 'exact' : ''
      } text matching '${expected}',` + ` but had '${actual}'`;

    return { pass, message };
  }

  const pass =
    exact && !Array.isArray(expected)
      ? actual === expected
      : actual.indexOf(expected) !== -1;
  const message = () =>
    `Expected element${pass ? ' not' : ''} to have ${
      exact ? 'exact' : ''
    } text '${expected}', but had '${actual}'`;
...
export const toHaveLength = comparator((el: string, expected: number) => {
  ...
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to have length ${expected}, but had ${actual}`;
...
export const toHaveId = comparator((el, expected) => {
  ...
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to have ID '${expected}', but had '${actual}'`;
 ...
 */
export const toHaveClass = comparator(
  (
    el,
    ...
    if (expected && Array.isArray(expected)) {
      ...
      const pass = options.strict
        ? $(el).hasClass(expectedClasses)
        : expected.every((e) => $(el).hasClass(e));
      const message = () =>
        `Expected element${
          pass ? ' not' : ''
        } to have value '${expectedClasses}', but had '${actual}'`;
    ...
    const message = () =>
      `Expected element${
        pass ? ' not' : ''
      } to have class '${expected}', but had '${actual}'`;
...
export const toHaveAttribute = comparator((el, attr: string | object, val) => {
  if (isObject(attr)) {
    ...
    for (const key of Object.keys(attr)) {
      ...
      const addendum =
        attr[key] !== undefined ? ` with value '${attr[key]}'` : '';
      pass = hasProperty(actual, attr[key]);
      failing = !pass ? `'${attr}'${addendum}, but had '${actual}'` : '';
    }
    const message = () =>
      `Expected element${pass ? ' not' : ''} to have attribute ${failing}`;
  ...
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to have attribute '${attr}'${addendum}, but had '${actual}'`;
...
export const toHaveProperty = comparator((el, prop, val) => {
  ...
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to have property '${prop}'${addendum}, but had '${actual}'`;
 ...
 */
export const toHaveText = comparator((el, expected, exact = false) =>
  hasSameText(el, expected, exact)
);

export const toHaveExactText = comparator((el, expected) =>
  hasSameText(el, expected, true)
);
...
export const toHaveValue = comparator((el, expected) => {
  if (expected && Array.isArray(expected)) {
    ...
    const message = () =>
      `Expected element${
        pass ? ' not' : ''
      } to have value '${failing}', but had '${actual}'`;
  ...
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to have value '${expected}', but had '${actual}'`;
...
export const toHaveStyle = comparator((el, expected) => {
  ...
  const message = () =>
    `Expected element${pass ? ' not' : ''} to have CSS ${JSON.stringify(
      expected
    )}`;
...
export const toHaveData = comparator((el, { data, val }) => {
  ...
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to have data '${data}'${addendum}, but had '${actual}'`;
...
export const toBePartial = comparator((actual, expected) => {
  ...
  const pass = expectedProps.every(
    (expectedProp) => actual[expectedProp] === expected[expectedProp]
  );
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to contain properties: ${JSON.stringify(expectedPropsAndValues)}.`.concat(
...
export const toBeMatchedBy = comparator((el, expected) => {
  ...
  const pass = actual > 0;
  const message = () =>
    `Expected element${pass ? ' not' : ''} to be matched by '${expected}'`;
...
export const toHaveDescendant = comparator((el, selector) => {
  ...
  const pass = actual > 0;
  const message = () =>
    `Expected element${pass ? ' not' : ''} to contain child '${selector}'`;
...
export const toHaveDescendantWithText = comparator((el, { selector, text }) => {
  ...
  if (text && $.isFunction(text.test)) {
    ...
    const message = () =>
      `Expected element${
        pass ? ' not' : ''
      } to have descendant '${selector}' with text matching '${text}',` +
      ` but had '${actual}'`;
  ...
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to have descendant '${selector}' with text '${text}', but had '${actual}'`;
...
export const toHaveSelectedOptions = comparator((el, expected) => {
  if (expected instanceof HTMLOptionElement) {
    ...
    const message = () =>
      `Expected element${pass ? ' not' : ''} to have options '[${
        expected.outerHTML
      }]' but had '[${actual[0].outerHTML}]'`;
  ...
  if (isHTMLOptionElementArray(expected)) {
    ...
    const pass =
      ...
      actual
        .toArray()
        .every((_, index) => $(actual[index]).is(expected[index]));
    ...
    const message = () =>
      `Expected element${
        pass ? ' not' : ''
      } to have options '[${expectedOptionsString}]' but had '[${actualOptionsString}]'`;
  ...
  const expectedOptionsString = Array.isArray(expected)
    ? expected.reduce(
        (acc: string, val: string, i) =>
          acc + `${i === expected.length ? '' : ','}${val}`
      )
    : expected;
  const message = () =>
    `Expected element${
      pass ? ' not' : ''
    } to have options '[${expectedOptionsString}]' but had '[${actual}]'`;
